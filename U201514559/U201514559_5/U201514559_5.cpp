...
/*******************STACK类结束*******************/
int x = 0;//元素下标
ofstream MyFile("U201514559_5.txt");//文件
int opt_flag = 0;//操作标志
/*******************QUEUE类定义*******************/
class QUEUE{
	STACK s1, s2;//s1为入队，栈顶表示队尾；s2为出队，栈顶表示队首。
public:
...
QUEUE::QUEUE(const QUEUE&s):s1(s.s1), s2(s.s2)//用队列s拷贝初始化队列
{
}
...
{
	if(int(*this)>=s1.size()+s2.size())//判断当前队列空间与最大空间大小
	{
		return 1;
	}
	return 0;
...
{
	int p = 0;
    if ((s1<<e).size() != 0)//如果栈s1不满，直接入栈s1中
    	return *this;
    else
...
QUEUE& QUEUE::operator>>(int &e)//出队列到e,并返回队列
{
	...
	if ((s2>>e).size() != 0)//如果s2不为空，直接弹出
    	...
    	if(int(s1)!=0)//s2为空，s1不为空，则把s1中的元素弹到s2中，再从s2出栈
    	{
    		while((s1>>p).size()!=0)
    		{
    			s2<<p;
...
void QUEUE::print() const//打印队列
{
	...
	if (opt_flag <5)//完成的操作为I 0 C A
	{
		...
		if(int(s1)!=0)//s2中所有元素输出结束后，若s1不为空，输出
		{
			for (i = 0 ; i < int(s1); i++)//栈底的下标为0,从栈顶输出到栈底，s1的栈顶为队列尾
			{
				cout << "  " << s1[i];
				MyFile << "  " << s1[i];
			}
